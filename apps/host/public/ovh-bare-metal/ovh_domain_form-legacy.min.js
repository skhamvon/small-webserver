;(function(){System.register([],function(){'use strict';return{execute:function(){/** Highest positive signed 32-bit float value */const maxInt=2147483647;// aka. 0x7FFFFFFF or 2^31-1
...
let q=delta;for/* no condition */(let k=base;;k+=base){const t=k<=bias?tMin:k>=bias+tMax?tMax:k-bias;if(q<t){break;}const qMinusT=q-t;const baseMinusT=base-t;output.push(stringFromCharCode(digitToBasic(t+qMinusT%baseMinusT,0)));q=floor(qMinusT/baseMinusT);}output.push(stringFromCharCode(digitToBasic(q,0)));bias=adapt(delta,handledCPCountPlusOne,handledCPCount===basicLength);delta=0;++handledCPCount;}}++delta;++n;}return output.join('');};/**
			 ...
			 * email address.
			 */const toASCII=function(input){return mapDomain(input,function(string){return regexNonASCII.test(string)?'xn--'+encode(string):string;});};(Drupal2=>{const domainSearchForm={variables:{extension:document.querySelector("[data-ods-domain-form-extension]")?.getAttribute("data-ods-domain-form-extension")??""},constants:{searchUrl:null},elements:{searchInput:document.querySelector("[data-domain-input]"),formWrapper:document.querySelector("[data-ods-domain-form]"),wrapper:document.querySelector("[data-domain-wrapper]"),form:document.querySelector("[data-domain-wrapper] form"),getAllSearchInputs:()=>document.querySelectorAll("[data-domain-input]"),getAllFormWrappers:()=>document.querySelectorAll("[data-ods-domain-form]"),getAllWrappers:()=>document.querySelectorAll("[data-domain-wrapper]"),getAllForms:()=>document.querySelectorAll("[data-domain-wrapper] form")},methods:{initialize:()=>{domainSearchForm.methods.setPostUrl();domainSearchForm.events.formSubmit();},emptySearchInput:()=>{domainSearchForm.elements.searchInput.innerHTML="";domainSearchForm.elements.searchInput.value="";},setPostUrl:()=>{domainSearchForm.constants.searchUrl=domainSearchForm.elements.form?.getAttribute("action");if(domainSearchForm.constants.searchUrl){domainSearchForm.elements.form?.setAttribute("data-search-url",domainSearchForm.constants.searchUrl);}},getDomainValues:specificInput=>{const inputElement=specificInput||domainSearchForm.elements.searchInput;const inputValue=inputElement?.value;let domains="";const values=inputValue?.split(/[, \n]/);values?.forEach((value,index)=>{if(value!==""){value=domainSearchForm.methods.cleanDomain(value);domains+=domainSearchForm.methods.isDomainValid(value)?value:value+domainSearchForm.variables.extension;if(values.length-1!==index){domains+=",";}}});return domains;},setFormDomainValues:(domains,specificForm)=>{const form=specificForm||domainSearchForm.elements.form;const searchUrl=form?.getAttribute("action")||domainSearchForm.constants.searchUrl;let action=searchUrl;if(domains.length>0){action=`${action}?domain=${domains.toLowerCase()}`;}if(action&&form){form.setAttribute("action",action);}},isDomainValid:domain=>{const regex=/^((?!-))(xn--)?[a-z0-9][a-z0-9-_]{0,61}[a-z0-9]{0,1}\.(xn--)?([a-z0-9\-]{1,61}|[a-z0-9-]{1,30}\.[a-z]{2,})$/;return domain.match(regex);},cleanDomain:domain=>{const cleanDomain=domain.toLowerCase().replace(/(\s|!|"|#|\$|%|&amp;|'|\(|\)|\*|\+|\`|\/|:|;|\&gt;|=|\&lt;|\?|@|\[|\]|\\|\^|\{|\||\}|~|,)/g,"").replace(/^www\./,"");return toASCII(cleanDomain);}},events:{formSubmit:()=>{const allForms=domainSearchForm.elements.getAllForms();allForms.forEach(form=>{if(!form.hasAttribute("data-domain-listener-attached")){form.setAttribute("data-domain-listener-attached","true");form.addEventListener("submit",e=>{const currentForm=e.target;const currentInput=currentForm.querySelector("[data-domain-input]");if(currentInput){const domains=domainSearchForm.methods.getDomainValues(currentInput);domainSearchForm.methods.setFormDomainValues(domains,currentForm);}});}});}}};Drupal2.behaviors.domainSearchForm={attach(){domainSearchForm.methods.initialize();}};})(Drupal);}};});})();
